				...
				<form action="/notice/insert.do?${_csrf.parameterName }=${_csrf.token}" method="post" id="noticeForm" enctype="multipart/form-data">
					...
					<div class="card-body">
						<div class="form-group">
							...
							<input type="text" value="${notice.boTitle }"
								id="boTitle" name="boTitle" class="form-control" placeholder="제목을 입력해주세요">
						</div>
						<div class="form-group">
							<label for="boContent">내용을 입력해주세요</label>
							<textarea id="boContent" class="form-control" rows="14"
								name="boContent">${notice.boContent }</textarea>
						</div>
						<div class="form-group">
...
$(function(){
	...
	$('.attachmentFileDel').on('click',function(){
		...
		let noticeFileNo = id.substring(idx + 1); // fileNo 얻어오기
		let ptrn = `<input type="hidden" name="delNoticeNo" value="%V">`;
